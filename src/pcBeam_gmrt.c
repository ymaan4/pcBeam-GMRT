...
void main (int argc, char *argv[])
{
  ...
  int ii,jj,kk, nchans1, nchans2, nchans, itemp;
...
//=====================================
  ...
  //printf("f1, t1, nchans, tsamp: %d %d    %d  %Lf\n",f1,t1,nchans,tsamp);
  //printf("f2, t2: %d %d\n",f2,t2);
  ...
  //printf("nsblk1, nsblk2, nsblk: %d %d   %d\n",nsblk1, nsblk2, nsblk);
  dur1=dur2=0.0;
  ...
  while ((ns1=read_block(input1,nbits,fblock1,nsblk1))>0 && (ns2=read_block(input2,nbits,fblock2,nsblk2))>0){
      ...
      if(anorm==-9999.0){ // first block and uder didn't specify scaling
        tempblock=(float *) malloc((ns+5)*sizeof(float));
	...
	anorm = round(med_fact*10.0)/10.0;
      ...
      for (i=0; i<ns; i++) fblock[i] = fblock1[i] - anorm*fblock2[i];
      nout=ns;
      switch (obits) {
      case 32:
  ...
  printf("No. of Freq-channels in the PC-beam data: %d \n\n",nchans);
